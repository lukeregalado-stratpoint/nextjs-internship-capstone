...
export function ProjectCard({ project }: { project: ProjectCardData }) {
  ...
  return (
    <div className="relative bg-white dark:bg-outer_space-500 rounded-lg border border-french_gray-300 dark:border-paynes_gray-400 p-6 hover:shadow-lg transition-shadow">
      <div className="flex items-start justify-between mb-4">
        <div className="w-3 h-3 bg-blue_munsell-500 rounded-full mt-1" />
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <button
              ...
              className="p-1 rounded hover:bg-platinum-500 dark:hover:bg-paynes_gray-400"
            >
              <MoreVertical size={16} className="text-paynes_gray-500 dark:text-french_gray-400" />
            </button>
            {menuOpen && (
              <div className="absolute right-0 mt-1 w-36 bg-white dark:bg-outer_space-500 border border-french_gray-300 dark:border-paynes_gray-400 rounded-lg shadow-lg z-10">
                <button
                  onClick={() => {
                    setMenuOpen(false)
                    setEditOpen(true)
      ...
      <Link href={`/projects/${project.id}`}>
        <h3 className="text-lg font-semibold text-outer_space-500 dark:text-platinum-500 mb-2 hover:text-blue_munsell-500">
          {project.name}
        </h3>
      </Link>
      ...
      <div className="w-full bg-french_gray-300 dark:bg-paynes_gray-400 rounded-full h-2">
        <div
          className="bg-blue_munsell-500 h-2 rounded-full transition-all"
